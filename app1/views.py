...
def getfun(request,a,b):
    ...
    num = Person.objects.values('name').annotate(count=Count('article__id')).values('name','count')#作者和作者的文章数

    sql = Person.objects.values('name').annotate(count=Count('article__id')).values('name','count').query
    return render(request, 'getfun.html',{'sum':int(a)+int(b), 'zidian':zidian,'liebiao':range(1,100),'sjk':sjk,'sjk1':sjk1,'sjk2':sjk2,'num':num,'sql':sql})
